...
{
    [ContractClassFor(typeof(IMessagingClient))]
    internal abstract class ContractClassForIMessagingClient : IMessagingClient
    {
        public void SendMessage(string facility, Message message)
        {
            Contract.Requires(!string.IsNullOrWhiteSpace(facility));
            Contract.Requires(null != message);
            Contract.Requires(message.IsValid());
        }

        public Message ReceiveMessage(string facility, int waitTimeoutMs)
        {
            Contract.Requires(!string.IsNullOrWhiteSpace(facility));
            Contract.Requires(waitTimeoutMs >= -1);
